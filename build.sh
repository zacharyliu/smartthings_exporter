...
function run() {
  ...
  if [ -n "$TRAVIS" ]; then
    ...
    for f in *; do
      ...
      verbose "   --> $archivename"
      genericname="smartthings_exporter$extension"
      mv -f "$f" "$genericname"
